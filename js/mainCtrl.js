...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function() {
      ...
      itunesService.getMusic($scope.artist).then(function(data){
        var myFinalArray = [];
        var Info = function(Artist, Collection, AlbumArt, Type, CollectionPrice, Play, Song){
            this.Artist = Artist;
        ...
        for (var i = 0; i < data.length; i++) {
          var information = new Info(data[i].artistName, data[i].collectionName, data[i].artworkUrl100, data[i].kind, data[i].collectionPrice, data[i].previewUrl, data[i].trackName)
          myFinalArray.push(information)
          information = null
        }
  ...
  $scope.options = ['Artist', 'Collection', 'Type', 'Price', 'Song'];
